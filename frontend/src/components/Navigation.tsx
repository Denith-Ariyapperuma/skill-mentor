...
import { getMentorPortalContext } from "@/lib/api";
...
export function Navigation() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const t = await getToken({ template: "skillmentor-auth" }).catch(
          () => null,
        );
        if (!t || cancelled) return;
        const ctx = await getMentorPortalContext(t);
        if (!cancelled) setShowMentorNav(ctx !== null);
      } catch {
        if (!cancelled) setShowMentorNav(false);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const isAdmin =
    ...
    user.publicMetadata.role.toUpperCase() === "ADMIN";

...
export function MentorCard({ mentor }: MentorCardProps) {
  ...
  const primarySubject = mentor.subjects[0];
  const courseTitle = primarySubject?.subjectName ?? "";
  const courseImageUrl = primarySubject?.courseImageUrl ?? "";
  const courseDescription =
    primarySubject?.description?.trim().replace(/\r\n/g, "\n") ?? "";
    ...
    <>
      <Card className="flex flex-col h-full relative overflow-hidden group">
        <Link
          ...
          aria-label={`View profile of ${mentorName}`}
        />
            ...
            <div>
              ...
              {bio && bioTooLong && (
                <button
                  ...
                  className="text-primary text-sm font-medium mt-1 hover:underline pointer-events-auto"
                >
        ...
        <div className="p-6 pt-0 relative z-10 pointer-events-none">
          <Button
            type="button"
            onClick={(e) => {
              e.preventDefault();
            ...
            className="w-full bg-black text-white hover:bg-black/90 pointer-events-auto"
